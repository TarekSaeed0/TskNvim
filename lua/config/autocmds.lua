				...
				on_start = function()
					...
					for _, window in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
						...
						if config and (config.external or config.relative ~= "") then
							...
							highlights.NormalFloat = (window == current_window and vim.g.tsknvim_in_focus) and "NormalFloatC"
								or "NormalFloatNC"
						else
							...
							highlights.Normal = (window == current_window and vim.g.tsknvim_in_focus) and "NormalC" or "NormalNC"
						end

						highlights.LineNr = (window == current_window and vim.g.tsknvim_in_focus) and "LineNrC" or "LineNrNC"
						if vim.api.nvim_get_option_value("number", { win = window }) then
							vim.api.nvim_set_option_value(
								"relativenumber",
								(window == current_window and vim.g.tsknvim_in_focus),
								{ win = window }
							)
						end
			...
			for _, window in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
				if vim.api.nvim_get_option_value("number", { win = window }) then
					...
					vim.api.nvim_buf_set_extmark(
						buffer,
						vim.api.nvim_create_namespace("tsknvim_continue_statuscolumn_after_end_of_buffer"),
